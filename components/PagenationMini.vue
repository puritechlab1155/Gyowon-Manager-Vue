<template>
    <div class="pagenation flex justify-center items-center mt-4">
        <div class="flex items-center space-x-2 max-lg:space-x-1">
            <!-- 이전 버튼 -->
            <button @click="goToPrev" :disabled="currentPage === 1"
                class="px-1 py-1 text-[#727272] rounded-md flex items-center justify-center group disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <!-- 페이지 번호들 -->
            <div class="flex space-x-1 max-lg:space-x-2">
                <button v-for="page in visiblePages" :key="page" @click="goToPage(page)" :class="[
                    'w-8 h-8 flex items-center justify-center font-medium rounded-md',
                    currentPage === page
