<template>
    <div class="relative z-[110]">
                ...
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="w-64 pr-36 max-2xl:pr-4 max-2xl:w-40 max-sm:w-36 flex justify-start h-16 items-center gap-2 max-xl:gap-1 max-sm:pr-2 pl-8 max-xl:pl-4">
                        <ul>
                            <li class="flex flex-col items-center">
                                ...
                                :class="isSidemenuOpen ? 'bg-[#2B5BBB]' : 'bg-gray-300'"
                        ...
                        <div class="font-bold text-center text-[#2B5BBB] bg-[#EDF3FF] text-lg rounded-[15px] px-3 py-0.5">lms</div>
                    </div>
                    ...
                    <div class="flex justify-between items-center w-full">
                        ...
                        <div class="my-div relative flex items-center max-lg:justify-end w-[460px] h-10 border border-[2px] border-gray-200 rounded-xl bg-white banner-border max-md:hidden px-3 justify-between ">
                                ...
                                <div class="w-full h-full flex items-center banner-container ">
                                    ...
                                    >
                                        <div class="grid-cols-custom01"> 
                                            ...
                                            <span class="text-sm text-[#2B5BBB] font-semibold truncate">{{ item.value }}</span> 
                            ...
                            <div class="relative flex-shrink-0">
                                ...
                                <ul 
                                    v-show="dropdownOpen" 
                                    id="dropdownMenu"
                                    class="my-div absolute right-[-14px] max-xl:top-[32px] w-[100%] w-[460px] bg-white z-10 xl:mt-1 bg-white border border-[2px] border-gray-200 rounded-xl shadow-lg">
                                    <!-- 공통 스타일: grid 4col -->
                                    <li v-for="(item, idx) in bannerItems" :key="idx" class="px-6 py-2 hover:bg-[#E7F0FD] cursor-pointer sm:px-4">
                                        <a :href="item.link">
                                            <div class="grid-cols-custom02">
                                                ...
                                                <span class="text-sm text-[#2B5BBB] font-semibold col-span-1 truncate">{{ item.value }}</span>
                        ...
                        class="flex gap-2 pr-8 max-xl:pr-4 max-sm:gap-0 items-center">
                            ...
                            <NuxtLink to="/" class="lg:hidden block">
        ...
        v-if="visible"
        id="mobile-banner" 
        class="fixed top-[64px] z-[1000] md:hidden w-full bg-blue-500 text-white text-sm text-center py-2 px-4 left-0 shadow-md transition-all duration-300 h-[35px]"
        >          
                ...
                :style="{ transform: `translateX(-${currentBannerIndex * 100}%)` }">
                    ...
                    <div class="banner-container absolute top-0 left-0 w-full h-full flex items-center transition-transform duration-300 ease-in-out">
                        ...
                        class="banner-item text-white flex-shrink-0 w-full truncate overflow-hidden whitespace-nowrap`">   
                            <a :href="item.link" class="text-sm"><!-- 댄스화 주문/조회관리 페이지 이동 -->
            ...
            class="absolute left-0 w-screen bg-blue-500 border-blue-400 mt-1 overflow-hidden transition-all duration-500 ease-in-out"
            :class="{
            ...
            <!-- 배너 내용 -->
                <div 
                    ...
                    <a :href="item.link" class="text-sm flex items-center justify-center truncate overflow-hidden whitespace-nowrap">
                        <span>
                            <span class="date text-sm">{{ item.date }}</span>
                            <span class="pl-1 text-sm">{{ item.label }} |</span>
                            <span v-if="item.value" class="product-name text-sm">{{ item.value }}</span>
                            <span v-if="item.user" class="user-name text-sm pl-1">{{ item.user }}</span>
                        </span>
                    </a>
                </div>
            </div>
