<template>
    <div 
        ref="dropdownRef" 
        class="relative dropdown w-[140px]"
        @click.outside="closeDropdown">
            <button
            @click="toggleDropdown"
            class="dropdown-button bg-[#FFFFFE] border border-[#DBDEE3] text-[#727272] py-2 px-2 pr-2 rounded-md focus:outline-none focus:border-[#2B5BBB] h-[50px] w-full flex justify-between items-center"
            type="button"
            >
...
<script setup>
    ...
    const selectedLabel = computed(() => {
        return props.modelValue !== '' ? props.modelValue : '교육장소'
    })
